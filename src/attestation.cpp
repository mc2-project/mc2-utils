...
int Attestation::GetFormatSettings(
    ...
    if (oe_res != OE_OK) {
        spdlog::error("Failed to initialize verifier when getting enclave format settings.");
        spdlog::error("Returned error: {}", to_string(oe_res));
        return -1;
    }

    // Use the plugin.
    oe_res = oe_verifier_get_format_settings(format_id, format_settings, format_settings_size);
    if (oe_res != OE_OK) {
...
#ifndef HOST
int Attestation::GenerateEvidence(
    const          oe_uuid_t* format_id,
    ...
    if (oe_res != OE_OK) {
        spdlog::error("Failed to initialize attester when generating attestation evidence.");
        spdlog::error("Returned error: {}", to_string(oe_res));
        return -1;
    }
    ...
    if (res != 0) {
        spdlog::error("Failed to hash TMS enclave public key during attestation evidence generation.");
        return res;
    }
    ...
    oe_claim_t claims[2] = {
        ...
        },
        {
            .name = claim2_name,
            .value = hash,
            .value_size = sizeof(hash)
        }
    };
    oe_res = oe_serialize_custom_claims(
        claims,
        2,
        &claims_buf,
        &claims_buf_size);
    if (oe_res != OE_OK) {
        spdlog::error("Failed to serialize custom claims during attestation evidence generation.");
        spdlog::error("Returned error: {}", to_string(oe_res));
        return -1;
    }

    // Generate evidence
    oe_res = oe_get_evidence(
        format_id,
        0,
        claims_buf,
        claims_buf_size,
...
int Attestation::AttestEvidence(
    const          oe_uuid_t* format_id,
...
#define free_and_return(val) {             \
    ...
    if ((claim = _find_claim(claims, claims_length, OE_CLAIM_SIGNER_ID)) ==
        nullptr)
    {
        spdlog::error("During evidence attestation, couldn't find enclave signer ID.");
        free_and_return(-1);
    };

    if (claim->value_size != OE_SIGNER_ID_SIZE) {
        spdlog::error("During evidence attestation, OE_CLAIM_SIGNER_ID size didn't match OE_SIGNER_ID_SIZE.");
        free_and_return(-1);
    }
    ...
    if ((claim = _find_claim(claims, claims_length, OE_CLAIM_PRODUCT_ID)) ==
        nullptr)
    {
        spdlog::error("During evidence attestation, couldn't find enclave product ID.");
        free_and_return(-1);
    };

    if (claim->value_size != OE_PRODUCT_ID_SIZE) {
        spdlog::error("During evidence attestation, OE_CLAIM_PRODUCT_ID size didn't match OE_PRODUCT_ID_SIZE.");
        free_and_return(-1);
    }

    if (*(claim->value) != 1) {
        spdlog::error("During evidence attestation, product ID verification failed.");
        free_and_return(-1);
    }
    ...
    {
        spdlog::error("During evidence attestation, couldn't find enclave security version.");
        free_and_return(-1);
    };

    if (claim->value_size != sizeof(uint32_t)) {
        spdlog::error("During evidence attestation, OE_CLAIM_SECURITY_VERSION size is incorrect.");
        free_and_return(-1);
    }

    if (*(claim->value) < 1) {
        spdlog::error("During evidence attestation, enclave security version verification failed.");
        free_and_return(-1);
    }
    ...
    {
        spdlog::error("During evidence attestation, couldn't find OE_CLAIM_CUSTOM_CLAIMS_BUFFER.");
        free_and_return(-1);
    }

    uint8_t hash[SHA_DIGEST_SIZE];
    if (m_crypto->Hash(pem_public_key, hash, pem_key_size) != 0) {
        spdlog::error("During evidence attestation, couldn't hash enclave public key.");
        free_and_return(-1);
    }
    ...
    {
        spdlog::error("During evidence attestation, failed to deserialize custom claims buffer.");
        free_and_return(-1);
    }
    ...
    {
        spdlog::error("During evidence attestation, the nonce received from the client as part of the custom claim was not the expected nonce.");
        free_and_return(-1);
    }
    ...
    {
        spdlog::error("During evidence attestation, the hash received from the client as part of the custom claim was not the expected hash.");
        free_and_return(-1);
    }
