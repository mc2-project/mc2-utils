...
Crypto::Crypto() : m_initialized(false) {
  ...
  res = mbedtls_ctr_drbg_seed(&m_ctr_drbg_context, mbedtls_entropy_func,
                              &m_entropy_context, (unsigned char*)seed.c_str(),
                              seed.size());
  if (res != 0) {
  ...
  res =
      mbedtls_rsa_gen_key(mbedtls_pk_rsa(m_pk_context), mbedtls_ctr_drbg_random,
                          &m_ctr_drbg_context, RSA_MOD_SIZE * 8, RSA_EXPONENT);
  if (res != 0) {
...
int Crypto::AsymEnc(const uint8_t* pem_public_key,
                    const uint8_t* data,
                    uint8_t* enc_data,
                    size_t data_size) {
  int res = -1;
...
int Crypto::AsymDec(const uint8_t* enc_data,
                    uint8_t* data,
  ...
  *data_size = 0;
  ...
  for (size_t i = 0; i < num_cts; i++) {
    ...
    res = mbedtls_rsa_rsaes_oaep_decrypt(
        rsa_context, mbedtls_ctr_drbg_random, &m_ctr_drbg_context,
        MBEDTLS_RSA_PRIVATE, aad.data(), aad.size(), &pt_size, next_ct, next_pt,
        RSA_MOD_SIZE);
    if (res != 0) {
...
int Crypto::SymEnc(const uint8_t* sym_key,
                   const uint8_t* data,
                   const uint8_t* aad,
                   uint8_t* enc_data,
                   size_t data_size,
...
int Crypto::SymDec(const uint8_t* sym_key,
...
int Crypto::Hash(const uint8_t* data,
...
int Crypto::sign_helper(mbedtls_pk_context* pk,
                        const uint8_t* data,
                        uint8_t* sig,
                        size_t data_size) {
  int res = -1;
...
#ifdef HOST
int Crypto::SignUsingKeyfile(const char* keyfile,
                             const uint8_t* data,
                             uint8_t* sig,
                             size_t data_size) {
  int res = -1;
...
int Crypto::Verify(const uint8_t* pem_public_key,
                   const uint8_t* data,
                   const uint8_t* sig,
                   size_t data_size) {
  int res = -1;
